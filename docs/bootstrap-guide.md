...
Run the following commands per environment. Replace `pre-prod` with `prod` for production.
...
aws dynamodb describe-table --table-name cdcu-terraform-locks-pre-prod --region ap-southeast-1
```
...
- The S3 bucket name must be globally unique. If `cdcu-terraform-state-pre-prod` is already taken, append the AWS account ID: `cdcu-terraform-state-pre-prod-{account_id}` and update `backend.tf` accordingly.
